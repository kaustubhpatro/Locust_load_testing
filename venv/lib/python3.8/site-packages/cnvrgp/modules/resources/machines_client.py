...
from cnvrgp.utils.validators import attributes_validator
...
from cnvrgp.modules.resources.machine import Machine
...
class MachinesClient:
    ...
    def get(self, slug):
        ...
        if not slug or not isinstance(slug, str):
            raise CnvrgError(error_messages.MACHINE_GET_FAULTY_SLUG)
    ...
    def create(
        ...
        **kwargs
        ...
        if not ssh_password and not ssh_key:
            raise CnvrgError(error_messages.MACHINE_NO_CREDENTIALS.format('create'))

        if not kwargs.get("gpu") and (kwargs.get("gpu_model") or kwargs.get('gpu_cores') or kwargs.get('gpu_ram')):
            raise CnvrgError(error_messages.MACHINE_GPU_VALUES_WITHOUT_GPU)
