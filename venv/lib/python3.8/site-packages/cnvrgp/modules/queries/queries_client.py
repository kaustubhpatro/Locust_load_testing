...
from cnvrgp.modules.queries.query import Query
...
class QueriesClient:
    ...
    def get(self, slug):
        ...
        if not slug or not isinstance(slug, str):
            raise CnvrgError(error_messages.QUERY_GET_FAULTY_SLUG)
    ...
    def create(self, name, query, commit_sha1=None):
        ...
        if not name or not isinstance(name, str):
            raise CnvrgError(error_messages.QUERY_CREATE_FAULTY_NAME)

        if not query or not isinstance(query, str):
            raise CnvrgError(error_messages.QUERY_CREATE_FAULTY_QUERY)
