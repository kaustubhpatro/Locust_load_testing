...
class MinioStorage(BaseStorageClient):
    ...
    def upload_single_file(self, local_path, object_path):
        try:
            self.client.upload_file(local_path, self.bucket, object_path, Config=config)
    ...
    def download_single_file(self, local_path, object_path):
        try:
            ...
            self.client.download_file(self.bucket, object_path, local_path, Config=config)
        except Exception as e:
            raise e

    def _get_client(self):
        botocore_config = botocore.config.Config(max_pool_connections=50)
        return boto3.client('s3', config=botocore_config, **self.s3props)
