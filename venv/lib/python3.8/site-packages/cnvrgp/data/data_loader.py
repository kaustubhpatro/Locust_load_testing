...
class DataLoader:
    def __init__(self, dataset, batch_size=1, num_workers=40, prefetch=3000, chunk_size=1000, force=False):
        ...
        self.storage_meta = dataset.storage_meta
        self.storage_client = storage_client_factory(storage_meta=self.storage_meta)

        # Handle batch input
        if not isinstance(batch_size, int) or batch_size < 1:
            raise CnvrgError("Batch sise must be an integer greater than 0")
        ...
        if not isinstance(prefetch, int) or prefetch < 1:
            raise CnvrgError("Prefetch factor must be an integer greater than 0")
        if not isinstance(chunk_size, int) or chunk_size < 1:
            raise CnvrgError("Chunk size must be an integer greater than 0")
        if not isinstance(num_workers, int) or num_workers < 1:
            raise CnvrgError("Number of workers must be an integer greater than 0")
