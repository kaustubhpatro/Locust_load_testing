...
from cnvrgp.data.local_files_handler import LocalFilesHandler
...
class FileUploader(LocalFilesHandler):
    ...
    def _handle_file_progress_function(self, uploaded_ids):
        ...
        self.data_owner._proxy.call_api(
            route=urljoin(self.data_owner._route, "commits", self.data_owner.current_commit, "upload_files_save"),
            http_method=HTTP.POST,
            payload=data
        )

    def _file_handler_function(self, local_path, object_path):
        """
    ...
    def _file_collector_function(self, files_metadata):
        ...
        response = self.data_owner._proxy.call_api(
            route=urljoin(self.data_owner._route, "commits", self.data_owner.current_commit, "upload_files"),
            http_method=HTTP.POST,
            payload=data
        )
